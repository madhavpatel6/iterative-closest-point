...
class MapBuilder:
    def __init__(self):
        ...
        self.pause_cmd_vel_publisher = rospy.Publisher('/joy_pause', Bool, queue_size=10)
        self.new_odom = None
        self.new_scan = None
        self.new_odomc = None
    ...
    def build_map(self):
        ...
        while not rospy.is_shutdown():
            if self.new_scan is not None and self.check_movement():
                ...
                self.icp_completed = True
                self.publish_map()
                self.pause_cmd_vel_publisher.publish(Bool(False))
                #self.laser_listen_once()
                #self.odom_listen_once()
...
def plot_points_2d(a, b):
    ...
    plt.clf()
    for p in a:
        plt.scatter(x=[p[0]], y=[p[1]], marker='o', color='r')
    for p in b:
        plt.scatter(x=[p[0]], y=[p[1]] , marker='x', color='b')
