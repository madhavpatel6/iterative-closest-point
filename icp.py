...
class MapBuilder:
    ...
    def laser_listen_once(self):
        self.subscriber = rospy.Subscriber('/cloud_out', PointCloud2, self.ros_callback)
    ...
    def build_map(self):
        ...
        while not rospy.is_shutdown():
            if self.new_scan is not None:
                if len(self.map) != 0:
                    new_scan_transformed = self.icp.iterative_closest_point(self.map, source=self.new_scan, initial=[1, 0, 0, 0, 0, 0, 0],
...
#                matrix[i][j] = self.compute_euclidean_distance_2d(a[i], b[j])):
    ...
    def nearest_neighbor_kdtree(self, a, b):
        ...
        for i in range(len(b)):
            new_cost, index = kdtree.query(b[i], k=1, distance_upper_bound=0.1)
